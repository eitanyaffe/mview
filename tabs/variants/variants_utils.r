...
# save variants table to tab-delimited file
save_variants_table <- function(variants_df, output_dir, region_name, use_simple_name = FALSE) {
  # create filename based on export type
  if (use_simple_name) {
    table_filename <- "variants.txt"
  } else {
    table_filename <- paste0(region_name, "_variants_table.txt")
  }
  ...
  if (is.null(variants_df) || nrow(variants_df) == 0) {
    ...
    cat(sprintf("saved empty variants table to: %s\n", filename))
    return(filename)
  }
  ...
  cat(sprintf("saved variants table (%d variants) to: %s\n", nrow(variants_df), filename))
  return(filename)
}
