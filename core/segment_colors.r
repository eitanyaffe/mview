...
.seg_colors_env$color_scheme <- "order_rainbow"
.seg_colors_env$user_schemes <- list()
.seg_colors_env$color_mappings <- list()

# dynamic schemes (always available)
.seg_colors_env$dynamic_schemes <- c("order_rainbow", "order_grayscale", "degree")
...
register_segment_colors <- function(schemes) {
  ...
  for (name in names(schemes)) {
    ...
    if (!is.character(scheme_val)) {
      stop(sprintf("scheme value for %s must be a character string (color field name)", name))
    }
...
# get color field name for a user scheme
get_user_scheme_color_field <- function(scheme_name) {
  if (scheme_name %in% names(.seg_colors_env$user_schemes)) {
    return(.seg_colors_env$user_schemes[[scheme_name]]$color)
  }
  return(NULL)
}
