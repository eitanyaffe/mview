...
set_regions_dir("configs/m21/regions")
...
aids <- c(sprintf("M%02d", 2:9), sprintf("M%d", 10:22))
...
view_file <- "configs/m21/m21_main_view.r"
...
get_map_tag <- function(assembly, timepoint) {
  cat("get_map_tag called with assembly:", assembly, "timepoint:", timepoint, "\n")
  cat("lib.table columns:", paste(names(lib.table), collapse=", "), "\n")
  cat("lib.table rows for assembly:\n")
  print(lib.table[lib.table$ASSEMBLY_ID == assembly, ])
  
  # m21 uses 'canon' column instead of 'SAMPLE_TYPE'
  ix = lib.table$ASSEMBLY_ID == assembly & lib.table$canon == timepoint
  cat("matching rows:", sum(ix), "\n")
  ...
  if (sum(ix) > 1) {
    cat("multiple matching rows found\n")
    return (NULL)
  }
  
  # for m21: lib IDs are in format "SID###_{assembly}{fecal_id}"
  # the tag should be "{assembly}_{lib_id}"
  lib_id <- lib.table$LIB_ID[ix]
  cat("lib_id from table:", lib_id, "\n")
  
  # check if lib_id already contains assembly prefix
  if (grepl(paste0("^", assembly, "_"), lib_id)) {
    # c60 style: lib_id is already "{assembly}_{timepoint}"
    tag <- lib_id
  } else {
    # m21 style: lib_id is full SID format, construct tag
    tag <- paste0(assembly, "_", lib_id)
  }
