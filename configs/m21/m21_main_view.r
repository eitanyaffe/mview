...
# get_map_tag is now defined in m21_cfg.r
...
make_align_profile <- function(timepoint) {
  align_profile(
    ...
    name = timepoint,
    aln_f = function() {
      cat("make_align_profile aln_f called for timepoint:", timepoint, "\n")
      cat("current assembly:", cxt_get_assembly(), "\n")
      tag <- get_map_tag(cxt_get_assembly(), timepoint)
      cat("got tag:", tag, "\n")
      if (is.null(tag)) {
        cat("tag is NULL, returning NULL\n")
        return(NULL)
      }
...
## ########################################################
## # other subject alignment profile (commented out for m21)
## ########################################################
## 
...
## ########################################################
## # synteny profiles (commented out for m21)
## ########################################################
## 
